...
export default function BookingsPage() {
  ...
  const checkConnections = async () => {
    ...
    try {
      ...
      console.log('HoneyBook not configured:', error);
      setHoneybookConnected(false);
    }
  ...
  const syncWithHoneyBook = async () => {
    ...
    try {
      // Check if HoneyBook is configured
      const apiKey = localStorage.getItem('honeybook_api_key');
      if (!apiKey) {
        setSyncStatus({
          type: 'error',
          message: 'HoneyBook API key not configured. Please add your API key in the HoneyBook Settings page to enable sync.'
        });
        setSyncing(false);
        return;
      }
      ...
      setSyncStatus({
        type: 'error',
        message: 'An error occurred while syncing with HoneyBook. Please check your connection and API key.'
      });
    } finally {
      setSyncing(false);
    }
